...
namespace KontrolSystem.TO2.Parser {
    ...
    public static class TO2ParserExpressions {
        ...
        private static readonly Parser<Expression> BreakExpression = Tag("break").Map((_, start, end) => new Break(start, end));

        private static readonly Parser<Expression> ContinueExpression = Tag("continue").Map((_, start, end) => new Continue(start, end));

        private static readonly Parser<Expression> Block = Char('{').Then(WhiteSpaces0).Then(DelimitedUntil(Alt<IBlockItem>(
            Expression,
            LineComment,
            VariableDeclaration,
            ReturnExpression,
        ...
        private static readonly Parser<List<Expression>> CallArguments = Char('(').Then(WhiteSpaces0).Then(DelimitedUntil(Expression, CommaDelimiter, WhiteSpaces0.Then(Char(')'))));
        ...
        private static readonly Parser<Expression> TupleCreate = DelimitedN_M(2, null, Expression, CommaDelimiter, "<expression>").
        ...
        private static readonly Parser<List<FunctionParameter>> LambdaParameters = Char('(').Then(WhiteSpaces0).Then(DelimitedUntil(LambdaParameter, CommaDelimiter, WhiteSpaces0.Then(Char(')'))));
        ...
        private static readonly Parser<Expression> BracketTerm = Expression.Between(Char('(').Then(WhiteSpaces0), WhiteSpaces0.Then(Char(')'))).Map((expression, start, end) => new Bracket(expression, start, end));
        ...
        private static readonly Parser<Expression> TermWithSuffixOps = Term.Fold0(SuffixOps, (target, suffixOp, start, end) => {
            switch (suffixOp) {
        ...
        private static readonly Parser<Expression> MulDivBinaryExpr = Chain(UnaryPrefixExpr, MulDivBinaryOp, (left, op, right, start, end) => new Binary(left, op, right, start, end));
        ...
        private static readonly Parser<Expression> AddSubBinaryExpr = Chain(MulDivBinaryExpr, AddSubBinaryOp, (left, op, right, start, end) => new Binary(left, op, right, start, end));
        ...
        private static readonly Parser<Expression> BITBinaryExpr = Chain(AddSubBinaryExpr, BITOp, (left, op, right, start, end) => new Binary(left, op, right, start, end));
        ...
        private static readonly Parser<Expression> CompareExpr = Chain(BITBinaryExpr, CompareOp, (left, op, right, start, end) => new Binary(left, op, right, start, end));
        ...
        private static readonly Parser<Expression> BooleanExpr = Chain(CompareExpr, BooleanOp, (left, op, right, start, end) => new BinaryBool(left, op, right, start, end));

        private static readonly Parser<Expression> IfBody = Alt(Expression, ReturnExpression, BreakExpression, ContinueExpression);
