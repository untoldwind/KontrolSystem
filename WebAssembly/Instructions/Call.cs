...
namespace WebAssembly.Instructions {
    ...
    public class Call : Instruction, IEquatable<Call> {
        ...
        /// <param name="other">The instruction to compare against.</param>
        /// <returns>True if they have the same type and value, otherwise false.</returns>
        public bool Equals(Call? other) => other != null && other.Index == this.Index;
        ...
        internal sealed override void Compile(CompilationContext context) {
            if (this.Index >= context.CheckedMethods.Length)
                throw new CompilerException(
                    $"Function for index {this.Index} requseted, the assembly contains only {context.CheckedMethods.Length}.");
            ...
            context.PopStackNoReturn(this.OpCode, paramTypes.Cast<WebAssemblyValueType?>().Reverse(),
                paramTypes.Length);
