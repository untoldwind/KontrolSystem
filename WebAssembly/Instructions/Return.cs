...
namespace WebAssembly.Instructions {
    ...
    public class Return : SimpleInstruction {
        ...
        internal sealed override void Compile(CompilationContext context) {
            ...
            Assert(returnsLength == 0 ||
                   returnsLength ==
                   1); //WebAssembly doesn't currently offer multiple returns, which should be blocked earlier.

            var stackCount = stack.Count;

            if (stackCount > returnsLength) {
                if (returnsLength == 0) {
                    for (var i = 0; i < stackCount - returnsLength; i++) context.Emit(OpCodes.Pop);
                } else {
            ...
            if (returnsLength == 1) context.PopStackNoReturn(OpCode.Return, returns[0]);
