...
namespace WebAssembly.Instructions {
    ...
    public class LocalGet : VariableAccessInstruction {
        ...
        public LocalGet(uint index) : base(index) {
        }

        internal LocalGet(Reader reader) : base(reader) {
        }

        internal sealed override void Compile(CompilationContext context) {
            if (this.Index >= context.CheckedLocals.Length)
                throw new System.IndexOutOfRangeException(
                    $"Attempt to get local at index {this.Index} but only {context.CheckedLocals.Length} {(context.CheckedLocals.Length == 1 ? "local was" : "locals were")} defined.");
            ...
            if (localIndex < 0) {
                ...
                default:
                    if (localIndex >
                        65534) // https://docs.microsoft.com/en-us/dotnet/api/system.reflection.emit.opcodes.ldloc
