...
namespace WebAssembly.Instructions {
    ...
    public class Float32CopySign : SimpleInstruction {
        ...
        internal sealed override void Compile(CompilationContext context) {
            context.PopStackNoReturn(OpCode.Float32CopySign, WebAssemblyValueType.Float32,
                WebAssemblyValueType.Float32);

            context.Emit(OpCodes.Call, context[HelperMethod.Float32CopySign, (helper, c) => {
                var builder = c.CheckedExportsBuilder.DefineMethod("☣ Float32CopySign",
                    CompilationContext.HelperMethodAttributes, typeof(float), new[] { typeof(float), typeof(float), });
